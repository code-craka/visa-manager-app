...
router.post('/', auth_js_1.verifyNeonAuth, async (req, res) => {
    try {
        ...
        const newTask = await db_js_1.default.query('INSERT INTO tasks (client_id, assigned_to, created_by, task_type, description, commission, due_date) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *', [client_id, assigned_to, user.id, task_type, description, commission, due_date]);
...
router.get('/', auth_js_1.verifyNeonAuth, async (req, res) => {
    ...
    `;
        ...
        const allTasks = await db_js_1.default.query(query, params);
...
router.put('/:id', auth_js_1.verifyNeonAuth, async (req, res) => {
    try {
        ...
        if (user.role === 'partner') {
            ...
            const updatedTask = await db_js_1.default.query('UPDATE tasks SET status = $1, notes = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $3 RETURNING *', [status, notes || task.notes, id]);
            ...
            const updatedTask = await db_js_1.default.query(`UPDATE tasks SET 
...
router.patch('/:id/complete', auth_js_1.verifyNeonAuth, async (req, res) => {
    try {
        const { id } = req.params;
       ...
       WHERE id = $2 RETURNING *`, [completion_notes, id]);
        res.json({
...
router.delete('/:id', auth_js_1.verifyNeonAuth, async (req, res) => {
    try {
        ...
        const deletedTask = await db_js_1.default.query('DELETE FROM tasks WHERE id = $1 RETURNING *', [id]);
...
router.get('/partner/:partnerId/commission-report', auth_js_1.verifyNeonAuth, async (req, res) => {
    try {
        ...
        const commissionData = await db_js_1.default.query(`SELECT 
       ...
       FROM tasks 
       WHERE assigned_to = $1`, [partnerId]);
        const monthlyData = await db_js_1.default.query(`SELECT 
